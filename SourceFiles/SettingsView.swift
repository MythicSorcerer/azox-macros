import SwiftUI
import KeyboardShortcuts

struct SettingsView: View {
    @EnvironmentObject private var settings: SettingsStore

    var body: some View {
        Form {
            Section("Shortcuts") {
                ShortcutRow(title: "Toggle Left Clicker", name: .toggleLeft)
                ShortcutRow(title: "Toggle Right Clicker", name: .toggleRight)
                ShortcutRow(title: "Toggle Both Clicker", name: .toggleBoth)
            }

            Section("Click Speed") {
                Picker("Rate Mode", selection: $settings.config.rateMode) {
                    ForEach(RateMode.allCases) { mode in
                        Text(mode.label).tag(mode)
                    }
                }

                RateModeEditor(config: $settings.config)

                WarningRow(maxCps: settings.config.maxCPS())
            }

            Section("Auto Stop") {
                Picker("Stop Mode", selection: $settings.config.stopMode) {
                    ForEach(StopMode.allCases) { mode in
                        Text(mode.label).tag(mode)
                    }
                }

                StopModeEditor(config: $settings.config)
            }

            Section("Quick Tips") {
                Text("Use fixed CPS + stop after time for the classic CPS + end time workflow.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
        .padding(16)
        .frame(minWidth: 520, minHeight: 520)
    }
}

private struct ShortcutRow: View {
    let title: String
    let name: KeyboardShortcuts.Name

    var body: some View {
        HStack {
            Text(title)
            Spacer()
            KeyboardShortcuts.Recorder(for: name)
        }
    }
}

private struct WarningRow: View {
    let maxCps: Double

    var body: some View {
        HStack(spacing: 8) {
            if maxCps > 100 {
                Image(systemName: "exclamationmark.triangle.fill")
                    .foregroundStyle(.orange)
                    .help("Warning: Click speeds above 100 CPS may cause apps to freeze.")
                Text("High click speed detected")
                    .foregroundStyle(.orange)
            } else {
                Text("Click speed OK")
                    .foregroundStyle(.secondary)
            }
        }
    }
}

private struct RateModeEditor: View {
    @Binding var config: ClickConfiguration

    var body: some View {
        switch config.rateMode {
        case .fixedCPS:
            LabeledContent("CPS") {
                TextField("CPS", value: $config.cps, format: .number)
                    .frame(width: 120)
            }
        case .fixedInterval:
            HStack {
                LabeledContent("Interval") {
                    TextField("Value", value: $config.intervalValue, format: .number)
                        .frame(width: 120)
                }
                Picker("", selection: $config.intervalUnit) {
                    ForEach(TimeUnit.allCases) { unit in
                        Text(unit.label).tag(unit)
                    }
                }
                .labelsHidden()
            }
        case .clicksPerDuration:
            HStack {
                LabeledContent("Clicks") {
                    TextField("Clicks", value: $config.clicksPerDuration, format: .number)
                        .frame(width: 120)
                }
                LabeledContent("Per") {
                    TextField("Value", value: $config.durationValue, format: .number)
                        .frame(width: 120)
                }
                Picker("", selection: $config.durationUnit) {
                    ForEach(TimeUnit.allCases) { unit in
                        Text(unit.label).tag(unit)
                    }
                }
                .labelsHidden()
            }
        case .randomInterval:
            HStack {
                LabeledContent("Min") {
                    TextField("Min", value: $config.randomIntervalMin, format: .number)
                        .frame(width: 120)
                }
                LabeledContent("Max") {
                    TextField("Max", value: $config.randomIntervalMax, format: .number)
                        .frame(width: 120)
                }
                Picker("", selection: $config.randomIntervalUnit) {
                    ForEach(TimeUnit.allCases) { unit in
                        Text(unit.label).tag(unit)
                    }
                }
                .labelsHidden()
            }
        case .randomCPS:
            HStack {
                LabeledContent("Min CPS") {
                    TextField("Min", value: $config.randomCpsMin, format: .number)
                        .frame(width: 120)
                }
                LabeledContent("Max CPS") {
                    TextField("Max", value: $config.randomCpsMax, format: .number)
                        .frame(width: 120)
                }
            }
        }
    }
}

private struct StopModeEditor: View {
    @Binding var config: ClickConfiguration

    var body: some View {
        switch config.stopMode {
        case .none:
            Text("No auto-stop configured.")
                .foregroundStyle(.secondary)
        case .afterTime:
            HStack {
                LabeledContent("Time") {
                    TextField("Value", value: $config.stopTimeValue, format: .number)
                        .frame(width: 120)
                }
                Picker("", selection: $config.stopTimeUnit) {
                    ForEach(TimeUnit.allCases) { unit in
                        Text(unit.label).tag(unit)
                    }
                }
                .labelsHidden()
            }
        case .afterClicks:
            LabeledContent("Clicks") {
                TextField("Clicks", value: $config.stopClicks, format: .number)
                    .frame(width: 120)
            }
        case .randomTimeRange:
            HStack {
                LabeledContent("Min") {
                    TextField("Min", value: $config.randomTimeMin, format: .number)
                        .frame(width: 120)
                }
                LabeledContent("Max") {
                    TextField("Max", value: $config.randomTimeMax, format: .number)
                        .frame(width: 120)
                }
                Picker("", selection: $config.randomTimeUnit) {
                    ForEach(TimeUnit.allCases) { unit in
                        Text(unit.label).tag(unit)
                    }
                }
                .labelsHidden()
            }
        case .randomClickRange:
            HStack {
                LabeledContent("Min") {
                    TextField("Min", value: $config.randomClicksMin, format: .number)
                        .frame(width: 120)
                }
                LabeledContent("Max") {
                    TextField("Max", value: $config.randomClicksMax, format: .number)
                        .frame(width: 120)
                }
            }
        }
    }
}
